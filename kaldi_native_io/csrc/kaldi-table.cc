...
bool ReadScriptFile(
  ...
  if (!input.Open(rxfilename, &is_binary)) {
    if (warn)
      KALDIIO_WARN << "Error opening script file: "
                   << PrintableRxfilename(rxfilename);
    return false;
  }
  if (is_binary) {
    if (warn)
      KALDIIO_WARN << "Error: script file appears to be binary: "
                   << PrintableRxfilename(rxfilename);
    return false;
  }
  ...
  if (warn && !ans)
    KALDIIO_WARN << "[script file was: " << PrintableRxfilename(rxfilename)
                 << "]";
  return ans;
}
...
bool ReadScriptFile(
    ...
    std::vector<std::pair<std::string, std::string>> *script_out) {
  KALDIIO_ASSERT(script_out != NULL);
  std::string line;
  int line_number = 0;
  while (getline(is, line)) {
    ...
    if (*c == '\0') {
      if (warn)
        KALDIIO_WARN << "Empty " << line_number << "'th line in script file";
    ...
    if (key.empty() || rest.empty()) {
      if (warn)
        KALDIIO_WARN << "Invalid " << line_number << "'th line in script file"
                     << ":\"" << line << '"';
      return false;
    }
...
bool WriteScriptFile(
  ...
  if (!os.good()) {
    KALDIIO_WARN << "WriteScriptFile: attempting to write to invalid stream.";
    return false;
  }
  std::vector<std::pair<std::string, std::string>>::const_iterator iter;
  for (iter = script.begin(); iter != script.end(); ++iter) {
    if (!IsToken(iter->first)) {
      KALDIIO_WARN << "WriteScriptFile: using invalid token \"" << iter->first
                   << '"';
      return false;
    }
    if (iter->second.find('\n') != std::string::npos ||
      ...
      KALDIIO_WARN << "WriteScriptFile: attempting to write invalid line \""
                   << iter->second << '"';
      return false;
    }
  ...
  if (!os.good()) {
    KALDIIO_WARN << "WriteScriptFile: stream in error state.";
    return false;
  }
  return true;
}
...
bool WriteScriptFile(
  ...
  if (!output.Open(wxfilename, false, false)) {  // false, false means not
    ...
    KALDIIO_ERR << "Error opening output stream for script file: "
                << PrintableWxfilename(wxfilename);
    return false;
  }
  if (!WriteScriptFile(output.Stream(), script)) {
    KALDIIO_ERR << "Error writing script file to stream "
                << PrintableWxfilename(wxfilename);
    return false;
  }
  return true;
