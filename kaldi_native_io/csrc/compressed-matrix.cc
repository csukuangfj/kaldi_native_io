...
MatrixIndexT CompressedMatrix::DataSize(const GlobalHeader &header) {
  ...
  if (format == kOneByteWithColHeaders) {
    ...
    return sizeof(GlobalHeader) + 2 * header.num_rows * header.num_cols;
  } else {
    KALDIIO_ASSERT(format == kOneByte);
    return sizeof(GlobalHeader) + header.num_rows * header.num_cols;
  }
}
...
void CompressedMatrix::ComputeGlobalHeader(const MatrixBase<Real> &mat,
                                           CompressionMethod method,
                                           GlobalHeader *header) {
  if (method == kAutomaticMethod) {
    if (mat.NumRows() > 8)
      method = kSpeechFeature;
    else
      method = kTwoByteAuto;
  }
  ...
  switch (method) {
    ...
    case kOneByteZeroOne:
      ...
      break;
    default:
      KALDIIO_ERR << "Invalid compression type: " << static_cast<int32>(method);
  }
  ...
  switch (method) {
    ...
    case kOneByteAuto: {
      ...
      // ensure that max_value is strictly greater than min_value, even if
      // matrix is constant; this avoids crashes in ComputeColHeader when
      // compressing speech featupres.
      if (max_value == min_value)
        max_value = min_value + (1.0 + fabs(min_value));
      KALDIIO_ASSERT(min_value - min_value == 0 && max_value - max_value == 0 &&
      ...
      KALDIIO_ASSERT(header->range > 0.0);
      break;
    }
    case kTwoByteSignedInteger: {
    ...
    default:
      KALDIIO_ERR << "Unknown compression method = "
                  << static_cast<int32>(method);
  }
  static_assert(sizeof(*header) == 20, "");  // otherwise
  // something weird is happening and our code probably won't work or
  // won't be robust across platforms.
}

template <typename Real>
void CompressedMatrix::CopyFromMat(const MatrixBase<Real> &mat,
                                   CompressionMethod method) {
  if (data_ != NULL) {
    delete[] static_cast<float *>(
        data_);  // call delete [] because was allocated with new float[]
    data_ = NULL;
  }
  if (mat.NumRows() == 0) {
    return;
  }  // Zero-size matrix stored as zero pointer.
  ...
  *(reinterpret_cast<GlobalHeader *>(data_)) = global_header;
  ...
  if (format == kOneByteWithColHeaders) {
    PerColHeader *header_data = reinterpret_cast<PerColHeader *>(
        static_cast<char *>(data_) + sizeof(GlobalHeader));
    uint8 *byte_data =
    ...
    for (int32 col = 0; col < global_header.num_cols; col++) {
      CompressColumn(global_header, matrix_data + col, mat.Stride(),
                     global_header.num_rows, header_data, byte_data);
      header_data++;
...
template void CompressedMatrix::CopyFromMat(const MatrixBase<float> &mat,
                                            CompressionMethod method);

template void CompressedMatrix::CopyFromMat(const MatrixBase<double> &mat,
                                            CompressionMethod method);

CompressedMatrix::CompressedMatrix(const CompressedMatrix &cmat,
  ...
  if (old_num_rows == 0) {
    KALDIIO_ASSERT(num_rows == 0 && num_cols == 0);
    // The empty matrix is stored as a zero pointer.
    return;
  }

  KALDIIO_ASSERT(row_offset < old_num_rows);
  KALDIIO_ASSERT(col_offset < old_num_cols);
  KALDIIO_ASSERT(row_offset >= 0 || allow_padding);
  KALDIIO_ASSERT(col_offset >= 0);
  KALDIIO_ASSERT(row_offset + num_rows <= old_num_rows || allow_padding);
  KALDIIO_ASSERT(col_offset + num_cols <= old_num_cols);

  if (num_rows == 0 || num_cols == 0) {
    return;
  }

  bool padding_is_used =
      (row_offset < 0 || row_offset + num_rows > old_num_rows);

  GlobalHeader new_global_header;
  static_assert(sizeof(new_global_header) == 20, "");

  GlobalHeader *old_global_header =
      reinterpret_cast<GlobalHeader *>(cmat.Data());
  ...
  *(reinterpret_cast<GlobalHeader *>(data_)) = new_global_header;
  ...
  if (format == kOneByteWithColHeaders) {
    ...
    uint8 *old_byte_data = reinterpret_cast<uint8 *>(
        old_per_col_header + old_global_header->num_cols);
    PerColHeader *new_per_col_header = reinterpret_cast<PerColHeader *>(
    ...
    if (!padding_is_used) {
      ...
      uint8 *old_start_of_col = old_byte_data + (col_offset * old_num_rows),
            *new_start_of_col = new_byte_data;
      for (int32 i = 0; i < num_cols; i++) {
        for (int32 j = 0; j < num_rows; j++) {
          ...
          if (old_j < 0)
            old_j = 0;
          else if (old_j >= old_num_rows)
            old_j = old_num_rows - 1;
    ...
    for (int32 row = 0; row < num_rows; row++) {
      ...
      if (old_row < 0)
        old_row = 0;
      else if (old_row >= old_num_rows)
        old_row = old_num_rows - 1;
  ...
  if (num_rows < 8 && format == kOneByteWithColHeaders) {
    ...
    Matrix<float> temp(this->NumRows(), this->NumCols(), kUndefined);
    this->CopyToMat(&temp);
...
// Instantiate the template for float and double.
template CompressedMatrix &CompressedMatrix::operator=(
    const MatrixBase<float> &mat);

template CompressedMatrix &CompressedMatrix::operator=(
    const MatrixBase<double> &mat);

inline uint16 CompressedMatrix::FloatToUint16(const GlobalHeader &global_header,
                                              float value) {
  float f = (value - global_header.min_value) / global_header.range;
...
inline uint8 CompressedMatrix::FloatToUint8(const GlobalHeader &global_header,
                                            float value) {
  float f = (value - global_header.min_value) / global_header.range;
  if (f > 1.0) f = 1.0;                      // Note: this should not happen.
  if (f < 0.0) f = 0.0;                      // Note: this should not happen.
...
inline float CompressedMatrix::Uint16ToFloat(const GlobalHeader &global_header,
                                             uint16 value) {
...
void CompressedMatrix::ComputeColHeader(
    const GlobalHeader &global_header, const Real *data, MatrixIndexT stride,
    int32 num_rows, CompressedMatrix::PerColHeader *header) {
  KALDIIO_ASSERT(num_rows > 0);
  ...
  if (num_rows >= 5) {
    ...
    header->percentile_25 = std::min<uint16>(
        std::max<uint16>(FloatToUint16(global_header, sdata[quarter_nr]),
                         header->percentile_0 + static_cast<uint16>(1)),
        65533);
    header->percentile_75 = std::min<uint16>(
        std::max<uint16>(FloatToUint16(global_header, sdata[3 * quarter_nr]),
                         header->percentile_25 + static_cast<uint16>(1)),
        65534);
    header->percentile_100 =
        std::max<uint16>(FloatToUint16(global_header, sdata[num_rows - 1]),
    ...
    if (num_rows > 3)
      header->percentile_100 = std::max<uint16>(
          FloatToUint16(global_header, sdata[3]), header->percentile_75 + 1);
    else
...
// static
inline uint8 CompressedMatrix::FloatToChar(float p0, float p25, float p75,
                                           float p100, float value) {
  int ans;
...
// static
inline float CompressedMatrix::CharToFloat(float p0, float p25, float p75,
                                           float p100, uint8 value) {
  if (value <= 64) {
...
void CompressedMatrix::CompressColumn(const GlobalHeader &global_header,
                                      ...
                                      uint8 *byte_data) {
  ComputeColHeader(global_header, data, stride, num_rows, header);
...
void CompressedMatrix::Write(std::ostream &os, bool binary) const {
  if (binary) {  // Binary-mode write:
    ...
    Matrix<float> temp_mat(this->NumRows(), this->NumCols(), kUndefined);
    this->CopyToMat(&temp_mat);
    temp_mat.Write(os, binary);
  }
  if (os.fail()) KALDIIO_ERR << "Error writing compressed matrix to stream.";
}
...
void CompressedMatrix::Read(std::istream &is, bool binary) {
  ...
  if (is.fail()) KALDIIO_ERR << "Failed to read data.";
}
...
void CompressedMatrix::CopyToMat(MatrixBase<Real> *mat,
  ...
  if (data_ == NULL) {
    KALDIIO_ASSERT(mat->NumRows() == 0);
    KALDIIO_ASSERT(mat->NumCols() == 0);
    return;
  }
  GlobalHeader *h = reinterpret_cast<GlobalHeader *>(data_);
  int32 num_cols = h->num_cols, num_rows = h->num_rows;
  KALDIIO_ASSERT(mat->NumRows() == num_rows);
  KALDIIO_ASSERT(mat->NumCols() == num_cols);

  DataFormat format = static_cast<DataFormat>(h->format);
  if (format == kOneByteWithColHeaders) {
    PerColHeader *per_col_header = reinterpret_cast<PerColHeader *>(h + 1);
    uint8 *byte_data = reinterpret_cast<uint8 *>(per_col_header + h->num_cols);
    ...
    float min_value = h->min_value, increment = h->range * (1.0 / 65535.0);
    for (int32 i = 0; i < num_rows; i++) {
...
// Instantiate the template for float and double.
template void CompressedMatrix::CopyToMat(MatrixBase<float> *mat,
                                          MatrixTransposeType trans) const;
template void CompressedMatrix::CopyToMat(MatrixBase<double> *mat,
                                          MatrixTransposeType trans) const;

template <typename Real>
void CompressedMatrix::CopyRowToVec(MatrixIndexT row,
                                    VectorBase<Real> *v) const {
  KALDIIO_ASSERT(row < this->NumRows());
  KALDIIO_ASSERT(row >= 0);
  KALDIIO_ASSERT(v->Dim() == this->NumCols());
  ...
  if (format == kOneByteWithColHeaders) {
    ...
    float min_value = h->min_value, increment = h->range * (1.0 / 65535.0);
    const uint16 *row_data =
        reinterpret_cast<uint16 *>(h + 1) + (num_cols * row);
    Real *v_data = v->Data();
    for (int32 c = 0; c < num_cols; c++)
...
void CompressedMatrix::CopyColToVec(MatrixIndexT col,
                                    VectorBase<Real> *v) const {
  KALDIIO_ASSERT(col < this->NumCols());
  KALDIIO_ASSERT(col >= 0);
  KALDIIO_ASSERT(v->Dim() == this->NumRows());
  ...
  if (format == kOneByteWithColHeaders) {
    ...
    uint8 *byte_data = reinterpret_cast<uint8 *>(per_col_header + h->num_cols);
    byte_data +=
        col * h->num_rows;  // point to first value in the column we want
    per_col_header += col;
    float p0 = Uint16ToFloat(*h, per_col_header->percentile_0),
          p25 = Uint16ToFloat(*h, per_col_header->percentile_25),
          p75 = Uint16ToFloat(*h, per_col_header->percentile_75),
...
template void CompressedMatrix::CopyColToVec(MatrixIndexT,
                                             VectorBase<double> *) const;
template void CompressedMatrix::CopyColToVec(MatrixIndexT,
                                             VectorBase<float> *) const;
template void CompressedMatrix::CopyRowToVec(MatrixIndexT,
                                             VectorBase<double> *) const;
template void CompressedMatrix::CopyRowToVec(MatrixIndexT,
                                             VectorBase<float> *) const;

template <typename Real>
void CompressedMatrix::CopyToMat(int32 row_offset, int32 col_offset,
                                 MatrixBase<Real> *dest) const {
  KALDIIO_PARANOID_ASSERT(row_offset < this->NumRows());
  KALDIIO_PARANOID_ASSERT(col_offset < this->NumCols());
  KALDIIO_PARANOID_ASSERT(row_offset >= 0);
  KALDIIO_PARANOID_ASSERT(col_offset >= 0);
  KALDIIO_ASSERT(row_offset + dest->NumRows() <= this->NumRows());
  KALDIIO_ASSERT(col_offset + dest->NumCols() <= this->NumCols());
  // everything is OK
  GlobalHeader *h = reinterpret_cast<GlobalHeader *>(data_);
  int32 num_rows = h->num_rows, num_cols = h->num_cols,
  ...
  if (format == kOneByteWithColHeaders) {
    ...
    // number of columns
    start_of_subcol +=
        col_offset * num_rows;  // skip appropriate number of rows

    per_col_header += col_offset;  // skip the appropriate number of headers

    for (int32 i = 0; i < tgt_cols;
...
// instantiate the templates.
template void CompressedMatrix::CopyToMat(int32, int32,
...
CompressedMatrix &CompressedMatrix::operator=(const CompressedMatrix &mat) {
  ...
  if (mat.data_ != NULL) {
    ...
    data_ = AllocateData(data_size);
    memcpy(static_cast<void *>(data_), static_cast<void *>(mat.data_),
           data_size);
  }
  return *this;
}
