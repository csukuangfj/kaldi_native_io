...
build_dir=$kaldi_native_io_dir/build
mkdir -p $build_dir
...
source $kaldi_native_io_dir/scripts/utils.sh
...
function do_check() {
  case "$1" in
    ...
    2)
      ...
      files=$(find $kaldi_native_io_dir/kaldi_native_io -name "*.h" -o -name "*.cc" -o -name "*.cu")
      ;;
    *)
